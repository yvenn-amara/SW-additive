...
df = read.csv("1. Preprocessed Data/palo_alto.csv")
...
prep = function(df,stations,trend=FALSE,weights=NULL,tzone){
  ...
  measurements = riem_measures(station = stations[[1]], 
                               date_start = format(min(with_tz(arrivals$Start,tzone="UTC") ) -24*3600, format = "%Y-%m-%d") , 
                               date_end = format(max(with_tz(arrivals$Start,tzone="UTC") ) +24*3600, format = "%Y-%m-%d") )
  
  dates_1min = with_tz(tibble(Start = seq(min(measurements$valid),max(measurements$valid),"1 min")),tzone='UTC')
...
# stations = list("HAF", "PAO", "SQL", "SFO")
# waights = weights=rep(0.25,4)

tzone="US/Pacific"
palo_alto = prep(df,stations="SQL",tzone=tzone)
...
# fmla_w = as.formula("event ~ factor(dow) + wp(tod) + wp(temp) + wp(toy)")
# fmla_s = as.formula("event ~ factor(dow) + s(tod,k=10) + s(temp,k=10) + s(toy, k=10)")
# fmla_sw = as.formula("event ~ factor(dow) + s(tod,k=10) + wp(tod) + s(temp,k=10) + wp(temp) + s(toy, k=10) + wp(toy)")
...
test_start = as.POSIXct("2019-01-07",tz=tzone)
# train_dates = list('2017-09-04')
# test_dates = list('2017-09-11')

training_routine = function(test_start,windows=3){
  metrics_train = NULL
  metrics_test = NULL
  for(i in 1:windows){
    slide1 = (i-1)*3600*24*7
    slide2 = i*3600*24*7
    train = filtered %>%
      filter(date < test_start + slide1)
    test = filtered %>%
      filter(date < test_start + slide2 & date >= test_start + slide1)
    ...
    pdf(file=paste0("5. Outputs/general/palo_alto/pred_plot",i,".pdf"),width=10,height=7)
...
met = training_routine(test_start,windows=20)

write.csv(met[[1]], "5. Outputs/general/palo_alto/metrics_train.csv",row.names=FALSE)
write.csv(met[[2]], "5. Outputs/general/palo_alto/metrics_test.csv",row.names=FALSE)
