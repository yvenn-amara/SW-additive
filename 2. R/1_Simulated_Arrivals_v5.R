##### Simulating Arrivals based on a known intensity function #####

##### 1. Importing functions libraries
library(tidyverse)
library(lubridate)
library(wavethresh)
library(gamwave)
library(glmnet)
library(ggpubr)
library(gridExtra)
library(mgcv)
require(fastDummies)
# args=commandArgs(TRUE)
# 
# step = as.numeric(args[1])
# num_weeks = as.numeric(args[2])
# lambda_num = as.numeric(args[3])
# iter = as.numeric(args[4])
step=15
num_weeks=1
lambda_num = 3
iter = 1

tod_res = 6
temp_res = 6
traffic_res = 6
points_simu = (7*1440/step)*num_weeks

dataset = "simulated_arrivals"
start_date = "2022-02-21"
source("2. R/0_Algos_Estim_NHPP_v3.R")
path = paste0("5. Outputs/",num_weeks,"_",lambda_num,"/")
dir.create(path)
#####

start_date = as.POSIXct(start_date, tz='UTC') 
set.seed(20201)
data = tibble(date = seq(start_date,by = paste(as.character(step),"mins"), length.out=points_simu)) %>%
  filter(wday(date) %in% c(2,3,4,5,6)) %>%
  mutate(t=seq(0,nrow(.)-1),
         t_day = t %% (1440/step),
         tod = floor((hour(date)/24 + minute(date)/(60*24)) * (2^tod_res)),
         # temp = -7.5*sin((2*pi/(1440/step))*t+pi/6)+10.5,
         temp = rnorm(nrow(.),15,3),
         ## traffic = -80*sin((2*pi/(1440/step))*(t+1000))+100,
         # t_traffic = (t_day-min(t_day))/(max(t_day)-min(t_day))*6-3,
         # traffic=100*exp(-abs(abs(t_traffic)-1)),
         traffic=rnorm(nrow(.),100,20),
         day = wday(date),
         indicator_mon = ifelse(wday(date) == 2,1,0),
         indicator_tue = ifelse(wday(date) == 3,1,0),
         # indicator_wed = ifelse(wday(date) == 4,1,0),
         indicator_thu = ifelse(wday(date) == 5,1,0),
         indicator_fri = ifelse(wday(date) == 6,1,0)
         )

lambdas = list(l_smooth(data,step=1),
               l_peaky(data,step=1),
               l_both(data,step=1),
               lambda_sim(data$t,step=step),
               lambda_sim2(data$t,step=step),
               lambda_sim3(data$t,step=step, peaks=2),
               lambda_sim3(data$t,step=step, peaks=5),
               lambda_sim3(data$t,step=step, peaks=10),
               lambda_peaky(data$t,step=step),
               lambda_mix(data$t,step=step))
data$lambda = lambdas[[lambda_num]]
# plot(data$date,data$lambda,type='l')
# plot(data$date,data$temp,type='l')
# plot(data$date,data$traffic,type='l')
# 
plot(data$date,lambdas[[1]],type='l')
plot(data$date,lambdas[[2]],type='l')
plot(data$date,lambdas[[3]],type='l')

pdf(file=paste0(path,"intensity",num_weeks,"_",lambda_num,".pdf"),width=9,height=5)
ggplot(data = data, aes(x=date, y = lambda)) + 
  geom_line()+
  ylab('Lambda') +
  xlab("Date") +
  theme_bw() +
  theme(legend.position="none") +
  ggtitle("")
dev.off()

plot(data$temp,data$lambda)
plot(data$traffic, data$lambda)
plot(data$tod, data$lambda)
plot(data$lambda,type='l')

wav_tod = WavD(data$tod, numLevels=tod_res, filterNumber = 1)
colnames(wav_tod) = paste0("wav_tod_",as.character(seq(1,2^tod_res-1)))
plot_wav(data$tod,numLevels = tod_res)

wav_temp = WavD(data$temp, numLevels=temp_res, filterNumber = 1)
colnames(wav_temp) = paste0("wav_temp_",as.character(seq(1,2^temp_res-1)))
plot_wav(data$temp,numLevels = temp_res)

wav_traffic = WavD(data$traffic, numLevels=traffic_res, filterNumber = 1)
colnames(wav_traffic) = paste0("wav_traffic_",as.character(seq(1,2^traffic_res-1)))
plot_wav(data$traffic,numLevels = traffic_res)

#####

simu = tibble(t_simu = thin_sim(data$lambda, ceiling(max(data['lambda'])), nrow(data), seed=24),
              event = 1) %>%
  inner_join(data%>%dplyr::select(date,t),.,by=c("t"="t_simu")) %>%
  dplyr::select(date,event) %>%
  group_by(date) %>%
  summarise_all(sum)

plot(simu$date,simu$event)

pdf(file=paste0(path,"simulation",num_weeks,"_",lambda_num,".pdf"),width=9,height=5)
ggplot(data = simu, aes(x=date, y = event)) + 
  geom_line()+
  ylab('Arrivals') +
  xlab("Date") +
  theme_bw() +
  theme(legend.position="none") +
  ggtitle("")
dev.off()


train = left_join(data,simu,by="date") %>%
  mutate(event = ifelse(is.na(event),0,event)) %>%
  cbind(.,wav_tod,wav_temp,wav_traffic) %>%
  mutate(pos = as.numeric(difftime(date,min(.$date),units="mins")),
         intercept = 1)

plot(train$date,train$event,type='l')

# train = left_join(data,
#                   tibble(as.data.frame(basis_tod)) %>% mutate(tod = sort(unique(data$tod))),
#                   by='tod'
#                   ) %>% 
#   mutate(pos = as.numeric(difftime(date,min(.$date),units="mins")),
#          intercept = 1)
  
# valid_nhpp(data,data$lambda)

#####

effects = list()
effects[[1]] = as.matrix(train %>% dplyr::select(starts_with("indicator")))
effects[[2]] = as.matrix(train %>% dplyr::select(starts_with("wav_tod")))
effects[[3]] = as.matrix(train %>% dplyr::select(starts_with("wav_temp")))
effects[[4]] = as.matrix(train %>% dplyr::select(starts_with("wav_traffic")))

X=as.data.frame(effects)
ncol(X)
qr(X)$rank
ncol(X) == qr(X)$rank

#### 000. 

extract_effects = function(fmla,data,res_max=6){
  tf = terms.formula(fmla,specials=c("s","w","factor"))
  if(attr(tf,"response")>0){
    lterms = rownames(attr(tf,'factors'))[-c(1,unlist(attr(tf, "specials")))]
  }else{
    lterms = rownames(attr(tf,'factors'))[-unlist(attr(tf, "specials"))]
  }
  sterms = rownames(attr(tf,'factors'))[attr(tf, "specials")$s]
  wterms = rownames(attr(tf,'factors'))[attr(tf, "specials")$w]
  fterms = rownames(attr(tf,'factors'))[attr(tf, "specials")$factor]
  ft = sub("\\).*", "", sub(".*\\(", "", fterms)) 
  
  if(length(lterms)>0){
    effect_lin = as.matrix(data[lterms])
  } else {
    effect_lin = NULL
  }
  
  effect_fac = NULL
  if (length(ft)>0){
    for (z in 1:length(ft)){
      effect_fac = cbind(effect_fac,as.matrix(dummy_cols(data[ft[[z]]],remove_first_dummy = TRUE,remove_selected_columns = TRUE)))
    }
  }
  
  effect_lin_full = cbind(effect_lin,effect_fac)

  
  wt = sub("\\).*", "", sub(".*\\(", "", wterms)) 
  if (length(wt>0)){
  effects_wav = list()
    for (i in 1:length(wt)){
      var = sub("\\).*", "", sub(".*\\(", "", wterms[i])) 
      series = data[[var]]
      
      
      res = res_max
      wav_effect = WavD(series, numLevels=res, filterNumber = 1)
      colnames(wav_effect) = paste0("wav_",var,"_",as.character(seq(1,2^res-1)))
      effects_wav[[i]] = wav_effect
      X=as.data.frame(wav_effect)
      
      while(ncol(X) != qr(X)$rank){
        print(res)
        res = res - 1
        wav_effect = WavD(series, numLevels=res, filterNumber = 1)
        colnames(wav_effect) = paste0("wav_",var,"_",as.character(seq(1,2^res-1)))
        effects_wav[[i]] = wav_effect
        X=as.data.frame(wav_effect)
      }
      
      

    }
  X = as.data.frame(effects_wav)
  if(ncol(X) == qr(X)$rank){print("Full Rank")}else{print("Not Full Rank")}
  } else{effects_wav = NULL}
  
  return(list("linear"=list(effect_lin_full),
              "splines"=sterms,
              "wavelets"=effects_wav))
}

rig_BAC2 = function(fmla,data, eps=0.001){
  effects = extract_effects(fmla,data,res_max = 8)
    return(rig_BAC(compact(c(effects$linear,effects$wavelets)),data,eps=eps))
}

rig_BACpen2 = function(fmla,data, eps=0.001){
  effects = extract_effects(fmla,data,res_max = 8)
  return(rig_BAC_pen(compact(c(effects$linear,effects$wavelets)),data,eps=eps))
}

fmla = as.formula("event ~ w(tod) + w(traffic) + w(temp)")
# outt = rig_BAC2(fmla,train, eps=1e-8)
# outtpen = rig_BACpen2(fmla,train, eps=1e-8)

fmla = as.formula("event ~ factor(day) + s(tod,k=10) + w(tod) + w(traffic) + w(temp)")
# fmla = as.formula("event ~ s(tod, k =20) + w(tod)")

outfinal = rig_BAC_final2(fmla,train, eps=1e-8, maxiter=1000)

plot(train$event,type='l')
lines(outfinal$lambdafit,col='blue')

outfinal$fullcoef


#### 00. Check if it works
out2 = rig_BAC(effects,train)
plot(out2$lambdafit)
lines(train$lambda)
mae(out2$lambdafit,train$lambda)
rmse(out2$lambdafit,train$lambda)

out = rig_BAC_pen(effects,train,maxiter=1000)
plot(out$lambdafit)
lines(train$lambda)
mae(out$lambdafit,train$lambda)
rmse(out$lambdafit,train$lambda)


fmla = as.formula("event ~ factor(day)")
start_time  = Sys.time()
out = BAC_GAM_then_wav(fmla,effects,train)
out_time  = difftime(Sys.time(),start_time,units = "secs")

#### 0. ALL AT ONCE

start_time  = Sys.time()
out_AAO = AAO(effects,train)
AAO_time  = difftime(Sys.time(),start_time,units = "secs")

AAO_tibble = data %>%
  dplyr::select(date) %>%
  mutate(lambda = out_AAO$lambdafit,
         model = "AAO")

start_time  = Sys.time()
out_AAO_pen= AA0_pen(effects,train)
AAO_pen_time  = difftime(Sys.time(),start_time,units = "secs")

AAO_pen_tibble = data %>%
  dplyr::select(date) %>%
  mutate(lambda = out_AAO_pen$lambdafit,
         model = "AAO_pen")

#### 1. OBO

out_OBO = OBO(effects,train)
out_OBO$metrics

out_OBO_pen = OBO_pen(effects,train)
out_OBO_pen$metrics

#### 2. BACKFITTING

start_time  = Sys.time()
out_BAC_GLM = BAC_GLM(effects,train, eps = 1e-8, maxiter = 100)
BAC_GLM_time  = difftime(Sys.time(),start_time,units = "secs")

out_BAC_GLM2 = BAC_GLM2(effects,train, eps = 1e-8, maxiter = 1000)


plot(out_BAC_GLM$metrics$rmse,type='l')
plot(out_BAC_GLM$metrics$mae,type='l')

BAC_GLM_tibble = data %>%
  dplyr::select(date) %>%
  mutate(lambda = out_BAC_GLM$lambdafit,
         model = "BAC")

start_time  = Sys.time()
out_BAC_GLMnet = BAC_GLMnet(effects,train, eps = 0.1, maxiter = 25)
BAC_GLMnet_time  = difftime(Sys.time(),start_time,units = "secs")

plot_wav(data$tod,numLevels = tod_res,filterNumber = 1, coef_is_null = out_BAC_GLMnet$fullcoef[[1]])

BAC_GLMnet_tibble = data %>%
  dplyr::select(date) %>%
  mutate(lambda = out_BAC_GLMnet$lambdafit,
         model = "BAC_pen")

aa = plot(out_BAC_GLMnet$cvfits[[1]], main='wav-tod')
bb = plot(out_BAC_GLMnet$cvfits[[2]], main='wav-temp')
cc = plot(out_BAC_GLMnet$cvfits[[3]], main='wav-traffic')
dd = plot(out_BAC_GLMnet$cvfits[[4]], main='linear')

if (iter = 1){
  pdf(file=paste0(path,"cvfits",num_weeks,"_",lambda_num,".pdf"),width=9,height=7)
  ggarrange(aa, bb, cc, dd,
            ncol = 2, nrow = 2)
  dev.off() 
}


plot(out_BAC_GLMnet$cvfits[[1]]$glmnet.fit)
abline(v = out_BAC_GLMnet$cvfits[[1]]$lambda.min*sum(abs(out_BAC_GLMnet$fullcoef[[1]])))

plot(out_BAC_GLMnet$cvfits[[2]]$glmnet.fit)
# abline(v = out_BAC_GLMnet$cvfits[[2]]$lambda)

out_BAC_GLMnet$fullcoef

#####

start_time  = Sys.time()
out_GAM = gam(event ~ s(tod,k=32) + s(temp,k=32) + s(traffic,k=32) + factor(day), data=train,family=poisson)
GAM_time  = difftime(Sys.time(),start_time,units = "secs")

GAM_tibble = data %>%
  dplyr::select(date) %>%
  mutate(lambda = predict(out_GAM,type="response"),
         model = "GAM")

# plot(data$lambda, type='l')
# lines(predict(out_GAM,type="response"),type='l', col='green')
# mae(data$lambda,predict(out_GAM,type="response"))
# out_GAM$coefficients

#####

start_time  = Sys.time()
out_glob = BAC_GLMnet_glob(effects_glm = effects,effects_gam = train %>% dplyr::select(tod),train)
glob_time  = difftime(Sys.time(),start_time,units = "secs")

GLOB_tibble = data %>%
  dplyr::select(date) %>%
  mutate(lambda = out_glob$lambdafit,
         model = "BAC_GW")

# plot(data$lambda, type='l')
# lines(out_glob$lambdafit,type='l', col='green')
# mae(data$lambda,out_glob$lambdafit)
# 
# out_glob$fullcoef

#####

aa = BAC_GAM_then_wav(as.formula("event ~ s(tod,k=32) + s(temp,k=32) + s(traffic,k=32) + factor(day)"),
                 effects,
                 train, eps=0.1)
  
bb = GAM_then_wav(as.formula("event ~ s(tod,k=32) + s(temp,k=32) + s(traffic,k=32) + factor(day)"),
                  effects,
                  train, eps=0.1)
plot(bb$lambdafit)
plot(bb$metrics)
#####

#### Saving results

metrics = tibble(model=c("AAO","AAO_pen","BAC","BAC_pen","GAM","BAC_GW"),
       mae=c(mae(data$lambda,out_AAO$lambdafit),
             mae(data$lambda,out_AAO_pen$lambdafit),
             mae(data$lambda,out_BAC_GLM$lambdafit),
             mae(data$lambda,out_BAC_GLMnet$lambdafit),
             mae(data$lambda,predict(out_GAM,type="response")),
             mae(data$lambda,out_glob$lambdafit)),
       rmse=c(rmse(data$lambda,out_AAO$lambdafit),
              rmse(data$lambda,out_AAO_pen$lambdafit),
              rmse(data$lambda,out_BAC_GLM$lambdafit),
              rmse(data$lambda,out_BAC_GLMnet$lambdafit),
              rmse(data$lambda,predict(out_GAM,type="response")),
              rmse(data$lambda,out_glob$lambdafit)),
       runtime=c(AAO_time,
                 AAO_pen_time,
                 BAC_GLM_time,
                 BAC_GLMnet_time,
                 GAM_time,
                 glob_time),
         param=c(sum(unlist(out_AAO$fullcoef)!=0),
                 sum(unlist(out_AAO_pen$fullcoef)!=0),
                 sum(unlist(out_BAC_GLM$fullcoef)!=0),
                 sum(unlist(out_BAC_GLMnet$fullcoef)!=0),
                 length(unlist(coef(out_GAM))!=0 ),
                 sum(unlist(out_glob$fullcoef)!=0)
                 ),
       iter = c(NA,
                NA,
                out_BAC_GLM$iterations,
                out_BAC_GLMnet$iterations,
                NA,
                out_glob$iterations)
       
       ) %>%
  mutate(runtime = as.numeric(runtime))

write.csv(metrics,paste0("5. Outputs/",num_weeks,"_",lambda_num,"/metrics_step",step,"_nweeks",num_weeks,"_nlambda",lambda_num,"_iter",iter,".csv"), row.names = F)

forplot = data %>%
  dplyr::select(date,lambda) %>%
  mutate(model="observed") %>%
  bind_rows(.,AAO_tibble,AAO_pen_tibble,BAC_GLM_tibble,BAC_GLMnet_tibble,GAM_tibble,GLOB_tibble) %>%
  filter(date <= min(date)+1440*5*60)
  
if (iter ==1){
  pdf(file=paste0("5. Outputs/",num_weeks,"_",lambda_num,"/lineplot_step",step,"_nweeks",num_weeks,"_nlambda",lambda_num,"_iter",iter,".pdf"),width=9,height=5)
  ggplot(data = forplot, aes(x=date, y = lambda)) + 
    geom_line(aes(color = model)) +
    theme_bw()
  dev.off()
}


##### END
